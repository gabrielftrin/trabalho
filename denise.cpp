#include #include <string.h> #include <stdlib.h> #include

/* 1- ler n na chamada do programa e cria vetor de tamanho 2- gera números aleatorios para vetor[n] (entre 0-100) 3- pesquisa sequencial 4- ordena vetor 5- pesquisa binaria 6- calcular tempo de busca seq e busca bin */
...
tamVetor = tamanho do vetor */ void geraNumerosRandomicos(int vet[], int tamVetor){ int i; //std::rand(); for (i = 0; i < tamVetor; i++){ cout<<"Os valores aleatorio gerados sao: ";
...
tamVetor = tamanho do vetor */ void buscaSequencial(int vet[], int tamVet){ clock_t tempo; // Variavel do tempo tempo = clock(); // Tempo inicial
...
tamVetor = tamanho do vetor */ void ordenaVetor(int vet[], int tamVetor){ int i, key, j;

for (i = 1; i < tamVetor; i++) { key = vet[i]; j = i - 1;
...
tamVetor = tamanho do vetor */ void buscaBinaria(int vet[], int tamVetor){ clock_t tempo; // Variavel do tempo tempo = clock(); // Tempo inicial
...
while((limiteInf <= limiteSup) && (vet[meio] != buscaNum)){ meio = (int)((limiteInf + limiteSup)/2);
...
tamVetor = tamanho do vetor */ void exibeVetor(int vet[], int tamVetor){ int i; cout<<("\nExibe vetor\n")<<endl; for (i = 0; i < tamVetor; i++)
cout<<(vet[i])<<endl;

cout<<("\n\n"); }

/* Função que exibe menu e retorna opção */ int opcaoMenu(){ int opcao; cout<<("\nMENU\n------------------------------------\n")<<endl; cout<<("1 - Pesquisa sequencial\n")<<endl; cout<<("2 - Pesquisa binaria\n")<<endl; cout<<("3 - Exibir vetor\n")<<endl; cout<<("0 - Sair\n")<<endl; cout<<("Digite a opcao: ")<<endl; //cout<<("opcao")<<endl; cout<<("\n-----------------------------------------\n");

return opcao;
}

int main(int argc, char *argv[]){ // Recebe parâmetro do tamanho do vetor na chamada do programa int n = atoi(argv[1]); // Converte o parâmetro d tamanho do vetor para inteiro
